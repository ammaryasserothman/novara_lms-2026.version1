...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const markLessonComplete = useCallback((courseId: string, lessonId: string) => {
    ...
    setEnrollments(prev => ({
      ...
      [courseId]: {
        ...
        // Note: Math.random() is faster than crypto.randomUUID() and safe enough for local mock IDs.
  ...
  // Optimization: getCourseProgress is wrapped in useCallback to prevent recreation on every render,
  // providing a stable reference to consumers.
  const getCourseProgress = useCallback((courseId: string) => {
    return enrollments[courseId]?.progress || 0;
  }, [enrollments]);

  // Optimization: Expensive array operations (map, filter, sort) are now memoized using useMemo.
  // This prevents recalculating recommended courses on every single render unless courses or enrollments change.
  const memoizedRecommendedCourses = useMemo(() => {
  ...
  // Optimization: useCallback preserves the existing function-based interface while returning the pre-computed array.
  const getRecommendedCourses = useCallback(() => {
    return memoizedRecommendedCourses;
  }, [memoizedRecommendedCourses]);
  ...
  // Optimization: Wrapping the provider value object in useMemo prevents widespread re-renders
  // in all child components that consume GlobalContext whenever unrelated state changes.
