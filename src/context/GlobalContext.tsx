...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Pre-calculate recommended courses to avoid redundant O(N) calculations
  const memoizedRecommendedCourses = useMemo(() => {
    ...
    for (let i = 0; i < courses.length; i++) {
      ...
      if (recommended.length >= 2) break; // Early return once we have 2!
  ...
  const getRecommendedCourses = useCallback(() => memoizedRecommendedCourses, [memoizedRecommendedCourses]);
  ...
  // Memoize the value object provided to React Contexts
