...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addNotification = React.useCallback((title: string, message: string, type: Notification['type']) => {
  ...
  const enrollInCourse = React.useCallback((courseId: string) => {
  ...
  const markLessonComplete = React.useCallback((courseId: string, lessonId: string) => {
  ...
  const toggleBookmark = React.useCallback((itemId: string) => {
    setBookmarks(prev =>
  ...
  const getCourseProgress = React.useCallback((courseId: string) => {
  ...
  const getNextLesson = React.useCallback((courseId: string): string | null => {
    const enrollment = enrollments[courseId];
    const course = courses.find(c => c.id === courseId);
