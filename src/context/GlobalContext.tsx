...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Performance: Pre-compute recommendations to prevent O(N log N) re-calculations on every render
  const recommendedCoursesMemo = useMemo(() => {
    // 1. Use Set for O(1) lookups instead of Arrays
    ...
    // 2. Single-pass lookup avoiding O(N log N) sort
    for (const course of courses) {
  ...
  // Performance: Preserve function API with useCallback
  ...
  // Performance: Memoize the entire context value to prevent unnecessary re-renders of subscribers
  // Although not all values are deeply memoized, it avoids re-creating the object strictly when not needed.
