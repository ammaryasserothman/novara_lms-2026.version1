...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const enrollInCourse = useCallback((courseId: string) => {
    ...
    addNotification('Course Enrolled', `You have started ${courseTitle}`, 'success');
  ...
  const memoizedRecommendations = useMemo(() => {
    ...
    const fallbackRecommendations: Course[] = [];

    for (const course of courses) {
      if (!enrolledIds.has(course.id)) {
        if (userCategories.has(course.category)) {
          recommendations.push(course);
          if (recommendations.length >= 2) break; // Only need 2
        } else if (fallbackRecommendations.length < 2) {
          fallbackRecommendations.push(course);
        }
      }
    }

    return [...recommendations, ...fallbackRecommendations].slice(0, 2);
  }, [enrollments, courses]);

  // Simple "AI" Recommendation Engine
  const getRecommendedCourses = useCallback(() => {
    return memoizedRecommendations;
