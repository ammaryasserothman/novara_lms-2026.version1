...
export const CalendarPage: React.FC = () => {
   ...
   // Filter Events (Optimized with useMemo)
   const filteredEvents = useMemo(() => {
      return EVENTS.filter(e => filter === 'all' || e.type === filter);
   }, [filter]);

   // Group events by day to replace O(N*D) filtering with O(1) map lookup
   ...
   // Memoize sorted deadlines to prevent O(N log N) recalculation on render
