...
export const CalendarPage: React.FC = () => {
   ...
   // Performance optimization: Pre-group events by day to avoid O(N*D) filtering in render loop
   ...
   // Events for the current month view (O(1) lookup instead of O(N) filter)
   const getEventsForDay = (day: number) => {
      return eventsByDay.get(day) || EMPTY_EVENTS;
   };

   // Wrap sorted deadlines in useMemo to avoid O(N log N) sort on every render
