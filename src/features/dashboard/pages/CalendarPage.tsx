...
export const CalendarPage: React.FC = () => {
   ...
   // Filter Events (Memoized to prevent O(N) recalculations on every render)
   const filteredEvents = React.useMemo(() => EVENTS.filter(e => filter === 'all' || e.type === filter), [filter]);

   // Events for the current month view: Pre-grouped by day to avoid O(N*D) filtering inside render loops
   ...
   // Memoized to prevent O(N log N) sorting on every render
