...
export const CalendarPage: React.FC = () => {
   ...
   // Filter Events
   // Performance: Memoize filtered events to prevent reference changes on every render
   const filteredEvents = useMemo(() => {
      return EVENTS.filter(e => filter === 'all' || e.type === filter);
   }, [filter]);

   // Events for the current month view
   // Performance: Pre-group events by day to avoid O(N*D) filtering inside render loop
