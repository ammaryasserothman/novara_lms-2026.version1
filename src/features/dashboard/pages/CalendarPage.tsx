...
export const CalendarPage: React.FC = () => {
   ...
   // Memoized filtering and grouping logic to avoid O(N*M) iteration
   const { eventsByDay, upcomingDeadlines } = useMemo(() => {
      const dayMap = new Map<number, typeof EVENTS>();
      const upcoming: typeof EVENTS = [];
      const todayMock = new Date(2024, 9, 20).getTime();
      ...
      for (const event of EVENTS) {
         ...
         if (event.date.getTime() >= todayMock) {
            upcoming.push(event);
         }
      }
      ...
      return { eventsByDay: dayMap, upcomingDeadlines: upcoming };
   }, [filter, currentDate]);
