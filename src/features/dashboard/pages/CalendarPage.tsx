...
export const CalendarPage: React.FC = () => {
   ...
   // Memoize the filtered list of events so it doesn't recalculate on every render unless 'filter' changes.
   const filteredEvents = React.useMemo(() => {
      return EVENTS.filter(e => filter === 'all' || e.type === filter);
   }, [filter]);

   // ⚡ Bolt Performance Optimization:
   // Pre-group events by day into a Map instead of repeatedly filtering the entire events array for every day of the month.
   // This reduces O(N * D) filtering overhead to an O(N) map construction followed by O(1) lookups per day.
   ...
   // ⚡ Bolt Performance Optimization:
   // Memoize upcoming deadlines which includes an expensive O(N log N) sort operation.
