...
export const NotificationsPage: React.FC = () => {
    ...
    const { filteredNotifications, unreadCount } = React.useMemo(() => {
        ...
        // What: Combined two separate array.filter() calls into one loop.
        // Why: Prevents multiple traversals over the same notifications array.
        // Impact: Reduces complexity from O(2N) to O(N) and limits array allocations.
        const filtered = [];
        let unread = 0;
        ...
        for (const n of notifications) {
            ...
            if (filter === 'unread') {
                if (!n.isRead) filtered.push(n);
            } else if (filter === 'mentions') {
                if (n.type === 'mention') filtered.push(n);
            } else {
                filtered.push(n);
            }
        }
