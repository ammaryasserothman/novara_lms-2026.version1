...
export const NotificationsPage: React.FC = () => {
    ...
    // Bolt Optimization: Calculate filtered list and unread count in a single pass
    // to reduce time complexity from O(2N) to O(N) and avoid recalculation on re-renders.
    const { filteredNotifications, unreadCount } = React.useMemo(() => {
        ...
        for (const n of notifications) {
            ...
            let include = true;
            if (filter === 'unread' && n.isRead) include = false;
            else if (filter === 'mentions' && n.type !== 'mention') include = false;
