...
export const Dashboard: React.FC = () => {
   ...
   // Logic: Get Active Course and other enrolled courses
   const { activeCourse, activeEnrollment, otherEnrolledCourses } = React.useMemo(() => {
      const enrolledCourseIds = Object.keys(enrollments);
      const activeKey = enrolledCourseIds.find(id => enrollments[id].status === 'in_progress') || enrolledCourseIds[0];

      const courseMap = new Map(courses.map(c => [c.id, c]));
      const active = activeKey ? courseMap.get(activeKey) : undefined;
      const activeEnroll = activeKey ? enrollments[activeKey] : null;

      const others: (Course & { progress: number })[] = [];
      for (const id of enrolledCourseIds) {
         if (id === activeKey) continue;
         const course = courseMap.get(id);
         if (course) {
            others.push({ ...course, progress: enrollments[id].progress });
         }
         if (others.length >= 2) break; // Early stop once display limit reached
      }
   ...
   return (
      <div className="space-y-8 font-sans text-slate-600 animate-fade-in">
         ...
         {/* --- STATS ROW --- */}
         <StatsOverview activeCourse={activeCourse} enrolledCourseCount={Object.keys(enrollments).length} />
