...
export const Dashboard: React.FC = () => {
   ...
   // Memoized to prevent O(N) recalculations on unrelated context updates (e.g. notifications)
   const { enrolledCourseIds, activeEnrollmentKey, activeCourse, activeEnrollment } = React.useMemo(() => {
      if (!user || !enrollments || !courses) {
         return { enrolledCourseIds: [], activeEnrollmentKey: undefined, activeCourse: undefined, activeEnrollment: null };
      }
      ...
      return { enrolledCourseIds: ids, activeEnrollmentKey: key, activeCourse: course, activeEnrollment: enrollment };
   }, [user, enrollments, courses]);

   // Logic: Get other enrolled courses (Max 2 for display)
   // Memoized to prevent unnecessary map/filter recalculations
   ...
   // Memoize recommendations. Depend directly on enrollments/courses to avoid stale data,
   // but omit getRecommendedCourses to prevent infinite re-renders from unstable function references.
